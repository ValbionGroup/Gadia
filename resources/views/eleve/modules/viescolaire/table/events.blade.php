...
<tr class="text-gray-700 dark:text-gray-400">
    ...
    <td class="px-4 py-3 text-sm">
        <a href="{{ route('eleve.viescolaire.events', ['id' => $event['linkId']]) }}">Détail</a>
    </td>
    <td class="px-4 py-3 text-sm">
        @switch($event["gravity"])
            @case(1)
                <span
                    class="px-2 py-1 font-medium leading-tight text-green-500 bg-green-100 rounded-full dark:bg-green-500 dark:text-green-100">
                    1
                </span>
                @break
            @case(2)
                <span
                    class="px-2 py-1 font-medium leading-tight text-yellow-500 bg-yellow-100 rounded-full dark:bg-yellow-500 dark:text-yellow-100">
                    2
                </span>
                @break
            @case(3)
                <span
                    class="px-2 py-1 font-medium leading-tight text-orange-500 bg-red-100 rounded-full dark:bg-orange-500 dark:text-red-100">
                    3
                </span>
                @break
            @case(4)
                <span
                    class="px-2 py-1 font-medium leading-tight text-red-500 bg-red-100 rounded-full dark:bg-red-500 dark:text-red-100">
                    4
                </span>
                @break
            @case(5)
